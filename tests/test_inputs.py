...
from sql_unit.core.models import DataSource, InputSpec, InputType
...
from sql_unit.inputs.setup import InputExecutor, InputSetup, InputValidator
...
class TestGivenClauseParsing:
    ...
    def test_parse_simple_cte(self):
        ...
        given = [{"cte": {"targets": ["users"], "rows": [{"id": 1, "name": "Alice"}]}}]
    ...
    def test_parse_simple_relation(self):
        ...
        given = [{"relation": {"targets": ["prod_users"], "replacement": "test_users"}}]
        specs = GivenClauseParser.parse_given_clause(given)
    ...
    def test_parse_jinja_context(self):
        ...
        given = [{"jinja_context": {"table_name": "users", "start_date": "2024-01-01"}}]
        specs = GivenClauseParser.parse_given_clause(given)
    ...
    def test_jinja_syntax_in_relation_targets_error(self):
        ...
        given = [{"relation": {"targets": ["{{ table_name }}"], "replacement": "test"}}]
...
class TestRelationInput:
    ...
    def test_relation_simple_substitution(self):
        ...
        spec = InputSpec(
            input_type=InputType.RELATION, targets=["prod_users"], replacement="test_users"
        )
    ...
    def test_relation_substitution_multiple_occurrences(self):
        ...
        spec = InputSpec(
            input_type=InputType.RELATION, targets=["orders"], replacement="test_orders"
        )
        relation_input = RelationInput(spec)
    ...
    def test_relation_substitution_with_schema_qualified_name(self):
        ...
        spec = InputSpec(
            input_type=InputType.RELATION, targets=["prod.users"], replacement="test.users"
        )
        relation_input = RelationInput(spec)
    ...
    def test_relation_substitution_empty_targets(self):
        ...
        spec = InputSpec(input_type=InputType.RELATION, targets=[], replacement="test_table")
        relation_input = RelationInput(spec)
...
class TestRelationSubstitutor:
    ...
    def test_apply_multiple_substitutions(self):
        ...
        spec2 = InputSpec(
            input_type=InputType.RELATION, targets=["prod_orders"], replacement="test_orders"
        )
        ...
        result_sql = RelationSubstitutor.apply_substitutions(
            original_sql, [relation_input1, relation_input2]
        )
    ...
    def test_apply_substitutions_order_matters(self):
        ...
        spec1 = InputSpec(input_type=InputType.RELATION, targets=["users"], replacement="u")
        # Second substitution replaces 'u' with 'test_users'
        spec2 = InputSpec(input_type=InputType.RELATION, targets=["u"], replacement="test_users")
...
class TestInputSetupIntegration:
    ...
    def test_setup_multiple_jinja_context_error(self):
        ...
        given = [{"jinja_context": {"var1": "val1"}}, {"jinja_context": {"var2": "val2"}}]
...
class TestInputExecutorIntegration:
    ...
    def test_executor_with_cte_and_relation(self):
        ...
        given = [
            {"cte": {"targets": ["temp_data"], "alias": "cte_data", "rows": [{"id": 1}]}},
            {"relation": {"targets": ["prod_users"], "replacement": "test_users"}},
        ]
    ...
    def test_executor_preserves_sql_structure(self):
        ...
        given = [{"cte": {"targets": ["users"], "alias": "test_users", "rows": [{"id": 1}]}}]
    ...
    def test_executor_with_jinja_renderer(self):
        ...
        given = [{"jinja_context": {"table_name": "users"}}]
    ...
    def test_executor_execution_order(self):
        ...
        given = [
            ...
            {"cte": {"targets": ["cte_users"], "alias": "cte_users", "rows": [{"id": 1}]}},
            {"relation": {"targets": ["prod_users"], "replacement": "test_users"}},
        ]
...
class TestInputValidatorIntegration:
    ...
    def test_validator_detects_cte_name_collision(self):
        ...
        given = [
            {"cte": {"targets": ["users"], "sql": "SELECT 1"}},
            {"cte": {"targets": ["users"], "sql": "SELECT 1"}},
        ]
    ...
    def test_validator_passes_with_different_cte_names(self):
        ...
        given = [
            {"cte": {"targets": ["users"], "alias": "test_users", "rows": [{"id": 1}]}},
            {"cte": {"targets": ["orders"], "alias": "test_orders", "rows": [{"order_id": 1}]}},
        ]
        ...
        assert setup  # Setup is valid after validation
...
class TestJinjaContextDataSource:
    ...
    def test_nested_cte_data_source(self):
        ...
        spec = {"cte": {"sql": "SELECT id, name FROM users"}}
    ...
    def test_nested_cte_with_explicit_alias(self):
        ...
        spec = {"cte": {"sql": "SELECT 1", "alias": "my_cte"}}
...
class TestJinjaContextInput:
    ...
    def test_context_with_scalar_variables(self):
        ...
        spec = InputSpec(
            ...
            jinja_context={"table_name": "users", "start_date": "2024-01-01", "limit": 100},
        )
    ...
    def test_context_with_nested_cte(self):
        ...
        spec = InputSpec(
            input_type=InputType.JINJA_CONTEXT,
            jinja_context={"users_data": {"cte": {"rows": [{"id": 1, "name": "Alice"}]}}},
        )
    ...
    def test_context_with_mixed_variables(self):
        ...
        spec = InputSpec(
            ...
            jinja_context={
                "table_name": "users",
                "users_data": {"cte": {"sql": "SELECT * FROM source_users"}},
                "limit": 100,
            },
        )
    ...
    def test_build_jinja_context_dict_scalars_only(self):
        ...
        spec = InputSpec(
            input_type=InputType.JINJA_CONTEXT, jinja_context={"table": "users", "count": 10}
        )
    ...
    def test_build_jinja_context_dict_with_nested_sources(self):
        ...
        spec = InputSpec(
            input_type=InputType.JINJA_CONTEXT,
            jinja_context={
                "my_users": {"cte": {"rows": [{"id": 1}], "alias": "users_cte"}},
                "table_name": "users",
            },
        )
    ...
    def test_get_scalar_vars(self):
        ...
        spec = InputSpec(
            input_type=InputType.JINJA_CONTEXT,
            jinja_context={"users": {"cte": {"sql": "SELECT 1"}}, "table": "users", "limit": 10},
        )
...
class TestJinjaContextCollisionDetection:
    ...
    def test_no_collision_with_different_names(self):
        ...
        jinja_spec = InputSpec(
            input_type=InputType.JINJA_CONTEXT,
            jinja_context={"my_users": {"cte": {"rows": [{"id": 1}], "alias": "users_cte"}}},
        )
        cte_spec = InputSpec(
            input_type=InputType.CTE,
            targets=["other_table"],
        ...
        # Should not raise
        JinjaContextCollisionDetector.check_collisions(jinja_input, [jinja_spec, cte_spec])

    def test_collision_with_derived_alias(self):
        """Test collision detection when Jinja variable matches explicit CTE alias."""
        jinja_spec = InputSpec(
            input_type=InputType.JINJA_CONTEXT,
            jinja_context={"my_alias": "some_value"},  # Variable named 'my_alias'
        )
        cte_spec = InputSpec(
            input_type=InputType.CTE,
            targets=["users"],
    ...
    def test_duplicate_jinja_variable_detection(self):
        ...
        jinja_spec = InputSpec(
            input_type=InputType.JINJA_CONTEXT, jinja_context={"var1": "value1", "var2": "value2"}
        )

        jinja_input = JinjaContextInput(jinja_spec)

        # Should not raise
        JinjaContextCollisionDetector.check_collisions(jinja_input, [jinja_spec])
...
class TestNestedJinjaContextIntegration:
        ...
        given = [
            {
                "jinja_context": {
                    "users_data": {
                        "cte": {"rows": [{"id": 1, "name": "Alice"}], "alias": "cte_users"}
                    },
                    "table_name": "users",
                }
            }
        ...
        given = [
            {
                "jinja_context": {
                    "limit_value": 10,
                    "my_cte": {"cte": {"rows": [{"id": 1}], "alias": "cte_data"}},
                }
            }
        ]
    ...
    def test_nested_cte_with_relation_substitution(self):
        ...
        given = [
            {
                "jinja_context": {
                    "temp_users": {"cte": {"sql": "SELECT * FROM prod_users", "alias": "temp_cte"}}
                }
            },
            {"relation": {"targets": ["prod_users"], "replacement": "test_users"}},
        ]
    ...
    def test_nested_sources_respect_targets(self):
        ...
        spec = InputSpec(
            input_type=InputType.JINJA_CONTEXT,
            jinja_context={"data": {"cte": {"rows": [{"id": 1}], "targets": ["custom_target"]}}},
        )
    ...
    def test_multiple_nested_ctes_same_jinja_context(self):
        ...
        spec = InputSpec(
            input_type=InputType.JINJA_CONTEXT,
            jinja_context={
                "users": {"cte": {"rows": [{"id": 1}], "alias": "users_cte"}},
                "orders": {"cte": {"rows": [{"order_id": 1}], "alias": "orders_cte"}},
                "limit": 100,
            },
        )
...
class TestGivenInputsSpec:
    ...
    def test_spec_multiple_ctes_in_single_test(self):
        ...
        given = [
            {"cte": {"targets": ["users"], "rows": [{"id": 1, "name": "Alice"}]}},
            {"cte": {"targets": ["orders"], "rows": [{"order_id": 1, "user_id": 1}]}},
        ]
    ...
    def test_spec_jinja_variable_name_stability(self):
        ...
        spec = InputSpec(
            input_type=InputType.JINJA_CONTEXT,
            jinja_context={"my_table": {"cte": {"rows": [{"id": 1, "name": "Alice"}]}}},
        )
    ...
    def test_spec_scalar_jinja_context_values(self):
        ...
        spec = InputSpec(
            input_type=InputType.JINJA_CONTEXT,
            jinja_context={"start_date": "2024-01-01", "limit_value": 100, "is_active": True},
        )
    ...
    def test_spec_empty_jinja_context_block(self):
        ...
        spec = InputSpec(input_type=InputType.JINJA_CONTEXT, jinja_context={})
    ...
    def test_spec_ast_based_identifier_matching(self):
        ...
        spec = InputSpec(
            input_type=InputType.RELATION, targets=["source_table"], replacement="test_table"
        )
    ...
    def test_spec_data_type_preservation(self):
        ...
        rows = [{"id": 1, "name": "Alice", "is_active": True, "score": 95.5}]
        spec = InputSpec(
            input_type=InputType.CTE,
            targets=["users"],
            data_source=DataSource(format="rows", content=json.dumps(rows)),
    ...
    def test_spec_given_section_processing(self):
        ...
        given = [{"cte": {"targets": ["users"], "rows": [{"id": 1}]}}]
    ...
    def test_spec_data_isolation_per_test(self):
        ...
        given1 = [{"cte": {"targets": ["users"], "rows": [{"id": 1}], "alias": "test1"}}]

        given2 = [{"cte": {"targets": ["users"], "rows": [{"id": 2}], "alias": "test2"}}]
    ...
    def test_spec_basic_jinja_context_with_nested_cte(self):
        ...
        spec = InputSpec(
            input_type=InputType.JINJA_CONTEXT,
            jinja_context={"my_table": {"cte": {"rows": [{"id": 1}]}}},
        )
    ...
    def test_spec_nested_data_sources_with_explicit_alias(self):
        ...
        spec = InputSpec(
            input_type=InputType.JINJA_CONTEXT,
            jinja_context={"my_data": {"cte": {"rows": [{"id": 1}], "alias": "custom_name"}}},
        )
    ...
    def test_spec_combining_top_level_and_jinja_bound_sources(self):
        ...
        given = [
            {"cte": {"targets": ["top_level"], "rows": [{"id": 1}], "alias": "top_cte"}},
            {"jinja_context": {"jinja_var": {"cte": {"rows": [{"id": 2}], "alias": "jinja_cte"}}}},
        ]
    ...
    def test_spec_unused_jinja_variables_allowed(self):
        ...
        spec = InputSpec(
            ...
            jinja_context={
                "unused_var": "value",
                "my_table": {"cte": {"rows": [{"id": 1}], "alias": "used_cte"}},
            },
        )
    ...
    def test_spec_relation_targets_with_jinja_syntax(self):
        ...
        spec = InputSpec(
            input_type=InputType.RELATION, targets=["{{ jinja_var }}"], replacement="test_table"
        )
...
class TestInputExpectationsIntegration:
    ...
    def test_integration_multiple_inputs_with_expectation(self):
        ...
        given = [
            {"cte": {"targets": ["users"], "rows": [{"id": 1}, {"id": 2}]}},
            {"relation": {"targets": ["prod_users"], "replacement": "test_users"}},
            {"jinja_context": {"table_name": "users"}},
        ]
    ...
    def test_integration_nested_jinja_with_row_count(self):
        ...
        given = [
            {
                "jinja_context": {
                    "orders_data": {
                        "cte": {
                            "rows": [{"order_id": 1, "user_id": 1}, {"order_id": 2, "user_id": 2}],
                            "alias": "orders_cte",
                        }
                    },
                    "threshold": 100,
        ...
        result_rows = [{"order_id": 1, "user_id": 1}, {"order_id": 2, "user_id": 2}]
    ...
    def test_integration_cte_and_relation_with_expectation(self):
        ...
        given = [
            ...
            {"relation": {"targets": ["prod.users"], "replacement": "staging_cte"}},
        ]
    ...
    def test_integration_all_input_types_with_validator(self):
        ...
        given = [
            {"cte": {"targets": ["users"], "rows": [{"id": 1}]}},
            {"cte": {"targets": ["orders"], "rows": [{"order_id": 1}]}},
            {"relation": {"targets": ["archive_users"], "replacement": "active_users"}},
            {"jinja_context": {"limit": 100}},
        ]
    ...
    def test_integration_csv_data_source_with_row_count(self):
        ...
        given = [{"cte": {"targets": ["users"], "csv": csv_data}}]
    ...
    def test_integration_validator_detects_conflicts(self):
        ...
        given = [
            {"cte": {"targets": ["users"], "rows": [{"id": 1}], "alias": "users_cte"}},
            {
                "cte": {
                    "targets": ["users"],
                    "rows": [{"id": 2}],
    ...
    def test_integration_expectation_max_with_result(self):
        ...
        given = [{"cte": {"targets": ["users"], "rows": [{"id": 1}, {"id": 2}]}}]
