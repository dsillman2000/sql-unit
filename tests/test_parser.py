...
class TestTestDefinitionBuilding:
    ...
    def test_build_single_test_definition(self):
        ...
        tests = SqlBlockCommentParser.build_test_definitions(parsed, "test.sql", 5)
        assert len(tests) == 1
    ...
    def test_build_with_auto_generated_name(self):
        ...
        parsed = [{"given": {}, "expect": {}}]
        tests = SqlBlockCommentParser.build_test_definitions(parsed, "test.sql", 1)
    ...
    def test_test_definition_test_id(self):
        """Test test_id() method."""
        test = TestDefinition(name="my_test", filepath="queries/test.sql")
