...
class DatabaseManager:
    ...
    def _create_engine(self) -> Engine:
        ...
        try:
            engine = create_engine(self.connection_string, poolclass=pool_class, echo=False)
            return engine
...
class ConnectionConfig:
    ...
    def sqlite(path: str = ":memory:", **options) -> "ConnectionConfig":
        ...
        return ConnectionConfig(
            database_type="sqlite", connection_string=conn_str, pooling=pooling, **options
        )
    ...
    def postgresql(
        ...
        **options,
        ...
        if database is None or user is None:
            raise ValueError("postgresql() requires 'database' and 'user' to be specified")
        url = ConnectionConfig._url(f"postgresql://{user}:{password}@{host}:{port}/{database}")
        return ConnectionConfig(
            database_type="postgresql", connection_string=url, pooling=True, pool_size=10, **options
        )
    ...
    def mysql(
        ...
        **options,
        ...
        """
        if database is None or user is None:
            raise ValueError("mysql() requires 'database' and 'user' to be specified")
        url = ConnectionConfig._url(f"mysql+pymysql://{user}:{password}@{host}:{port}/{database}")
        return ConnectionConfig(
            database_type="mysql", connection_string=url, pooling=True, pool_size=10, **options
        )
    ...
    def from_yaml(cls, config_dict: dict) -> "ConnectionConfig":
        ...
        if db_type == "sqlite":
            options = {
                key: value for key, value in config_dict.items() if key not in {"type", "path"}
            }
            return cls.sqlite(config_dict.get("path", ":memory:"), **options)

        elif db_type == "postgresql":
            options = {
                key: value
                for key, value in config_dict.items()
                if key not in {"type", "host", "port", "database", "user", "password"}
            }
            ...
            return cls.duckdb(config_dict.get("path", ":memory:"), **options)
    ...
    def create_engine(self) -> Engine:
        ...
        try:
            ...
            engine_kwargs = {"poolclass": pool_class, **self.kwargs}
