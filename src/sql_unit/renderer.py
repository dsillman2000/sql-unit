...
class SQLUnitEnvironment(Environment):
    """Custom Jinja2 environment for SQL rendering."""
...
class TemplateRenderer:
    ...
    def __init__(self, jinja_context: dict[str, Any] | None = None):
        ...
        self.env = SQLUnitEnvironment(
            trim_blocks=False, lstrip_blocks=False, undefined=StrictUndefined
        )

    def render(
        self, sql: str, test_id: str | None = None, additional_context: dict[str, Any] | None = None
    ) -> str:
        """
        ...
        try:
            ...
            raise RendererError(f"Jinja2 syntax error: {str(e)}", test_id=test_id, sql=sql)
        except UndefinedError as e:
            raise RendererError(
                f"Undefined variable in template: {str(e)}", test_id=test_id, sql=sql
            )
        except Exception as e:
            raise RendererError(f"Template rendering failed: {str(e)}", test_id=test_id, sql=sql)
